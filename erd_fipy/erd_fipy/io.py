...
class H5Writer:
    def __init__(self, outdir: str, run_name: str, r: np.ndarray, z: np.ndarray):
        ...
        self._Bz = self._h.create_dataset(
            "/fields/Bz", shape=(0, r.size), maxshape=(None, r.size), dtype="f8"
        )
        self._ne = self._h.create_dataset(
            "/fields/ne", shape=(0, z.size, r.size), maxshape=(None, z.size, r.size), dtype="f8"
        )
        self._Te = self._h.create_dataset(
            "/fields/Te", shape=(0, z.size, r.size), maxshape=(None, z.size, r.size), dtype="f8"
        )
        self._i = 0
    ...
    def write_snapshot(
        self, t: float, Bz_r: np.ndarray, ne_zr: np.ndarray, Te_zr: np.ndarray, inputs
    ):
